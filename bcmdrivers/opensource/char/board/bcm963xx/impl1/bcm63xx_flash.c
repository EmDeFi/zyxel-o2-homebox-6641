...
#if 1 //chenhe
extern uint8 LedBhOn[4];
extern uint8 LedCtlOn[4];
extern uint8 LedCtlOff[4];
extern void bcmsw_pmdio_wreg(int page, int reg, unsigned char *data, int len);
#endif
...
/* Bug fix, sts does not change to 0 and always stay at initial value , __CHT_P880__, MitraStar Ruki, 20111012. */
...
/*__MTS__, RaynorChung: Support 963268 nand flash, patch form SVN#3597 on http://svn.zyxel.com.tw/svn/CPE_SW1/BCM96368/trunk/P-870HA/branches/cht/fttb8/4.11*/
#ifdef MTS_NAND_FLASH_SUPPORT
int kerSysBcmImageSet( int flash_start_addr, char *string, int size, int* badblknum)
{
    int sts;
    int sect_size;
    int blk_start;
...
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120710*/
    ...
    /* LED Blinking when upgrade fw. MitraStar, Stan Su, 20111213. */
    int count = 0;
...
#if 1 /* ChenHe@MSTC for  F/W upgrade LED behavior */
	 	bcmsw_pmdio_wreg(0,26,LedBhOn,2);
	//	bcmsw_pmdio_wreg(0,18,LedCtlOff,2);
#endif
...
/*__MSTC__, Dennis*/
		...
		(*badblknum)++; //CLK
            ...
            size -= sect_size; 
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120710*/
#ifdef MTS_LED_CONTROL  /* ChenHe@MSTC for  F/W upgrade LED behavior */
	            /* LED Blinking when upgrade fw. MitraStar, Stan Su, 20111213. */
	            if(savedSize > sect_size) {
	            if (count % 20 >= 0 && count % 20 <= 9)
	            {
				kerSysLedCtrl(kLedPowerG, kLedStateOn);
				bcmsw_pmdio_wreg(0,18,LedCtlOn,2);
				kerSysLedCtrl(kLedAdsl_TO2, kLedStateOn);
				kerSysLedCtrl(kLedInternet, kLedStateOn);
				kerSysLedCtrl(kLedWanData, kLedStateOn);
				kerSysLedCtrl(kLedPhone, kLedStateOn);
				kerSysLedCtrl(kLedInfo, kLedStateOn);
				kerSysLedCtrl(kLedDect, kLedStateOn);
				kerSysLedCtrl(kLedWlan, kLedStateOn);
				...
				bcmsw_pmdio_wreg(0,18,LedCtlOff,2);
				kerSysLedCtrl(kLedAdsl_TO2, kLedStateOff);
				kerSysLedCtrl(kLedInternet, kLedStateOff);
				kerSysLedCtrl(kLedWanData, kLedStateOff);
				kerSysLedCtrl(kLedPhone, kLedStateOff);
				kerSysLedCtrl(kLedInfo, kLedStateOff);
				kerSysLedCtrl(kLedDect, kLedStateOff);
				kerSysLedCtrl(kLedWlan, kLedStateOff);
...
#endif //__MTS__, RaynorChung: Support 963268 nand flash, patch form SVN#3597 on http://svn.zyxel.com.tw/svn/CPE_SW1/BCM96368/trunk/P-870HA/branches/cht/fttb8/4.11
        ret = flash_get_total_size();

    return ret;
}
/*__MTS__, __VERIZON__, Allen*/
#ifdef MTS_NAND_FLASH_SUPPORT
#if !defined(CONFIG_BRCM_IKOS)
...
#endif // __MTS__, __VERIZON__, Allen

/*KeyYang@MSTC: Support 963268 nand flash, 20120701*/
#ifndef MTS_NAND_FLASH_SUPPORT
/***********************************************************************
...
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120808*/
        ...
        rootfsAddr = baseAddr + fInfo.flash_rootfs2_start_offset + TAG_BLOCK_LEN;
#else
...
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120808*/
#if (defined(MSTC_FWUP_FROM_FLASH) || defined(CONFIG_BUILD_MSTC_FWUP_FROM_FLASH))
        rootfsAddr = baseAddr + fInfo.flash_rootfs2_start_offset + TAG_BLOCK_LEN_NAND;
#else
